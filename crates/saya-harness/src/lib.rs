...
//! append-only event journal a resume replays. The fetch egress policy
//! (`fetch`) is the fail-closed decision of where a run may reach; it
//! performs no network I/O itself. The one database surface the run engine
