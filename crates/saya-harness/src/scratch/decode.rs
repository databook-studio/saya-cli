//! Scratch's value decoder: staged rows to JSON for the model. Re-implemented
//! on its own types, never shared with the connector's decoder (ADR 0003).
...
pub(super) fn json_value(value: ValueRef<'_>) -> Value {
...
fn json_owned(value: DuckValue) -> Value {
    match value {
        ...
        // Geometry arrives as well-known binary; hex matches how a blob is
        // rendered so a caller reads bytes the same way whichever column
        // produced them.
...
fn date(days: i32) -> String {
    NaiveDate::from_ymd_opt(1970, 1, 1)
        .and_then(|date| date.checked_add_signed(ChronoDuration::days(i64::from(days))))
