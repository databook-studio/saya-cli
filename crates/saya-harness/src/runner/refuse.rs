...
use saya_types::{RunnerScope, is_bare_name, is_refused_runner_program};
...
pub fn validate_call(
    ...
    if is_refused_runner_program(program) {
        return Err(RunnerError::ProgramRefused {
            program: program.to_owned(),
            reason: "shells and interpreters are refused by name: an interpreter can spawn \
                     arbitrary children with arbitrary argv and would void the typed-argv \
                     contract from inside the allowlist",
        });
    }
