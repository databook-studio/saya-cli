...
mod tests {
    ...
    fn the_show_stanza_names_cause_and_pause() {
        let text = run_show_text(
            "r-1",
            "failed",
            Some(crate::render_run::failure_code_cause(
                RunFailureCode::SafetyQuery,
            )),
            100,
            200,
            Some(("the goal", "workspace-write")),
            Some(PauseReason::StoreUnavailable),
        );
        assert_eq!(
            text,
