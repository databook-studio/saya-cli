...
async fn run_one_turn(mode: MemoryMode) -> (SqliteStateStore, ProfileIdentity, PathBuf) {
    ...
    run_agent(
        ...
        &tools,
        agent_request(),
        DatabaseTools::definitions(true, true, limits.permit_candidate_writes, false),
        limits,
...
async fn mode_assisted_recalls_candidates_and_permits_proposals() {
    ...
    let defs = DatabaseTools::definitions(true, true, setup.permit_candidate_writes, false);
    assert!(
...
async fn changing_memory_mode_takes_effect_on_the_next_turn() {
    ...
    assert!(!off.observes());
    let off_defs = DatabaseTools::definitions(true, true, off.permit_candidate_writes, false);
    assert!(
    ...
    let assisted_defs =
        DatabaseTools::definitions(true, true, assisted.permit_candidate_writes, false);
    assert!(
