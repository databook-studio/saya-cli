//! Bounding the rendered context block to what fits the conversation budget.
//!
...
//! the loop's `context_byte_budget` after the system prompt and the user's own
...
//! the end (least-relevant first, since `recall` ranks them) until the rendered
//! block fits, and if even the first contract does not fit it is omitted and the
//! block is marked truncated.
//!
...
use saya_agent::{ContextBlock, turn_bytes};
use std::collections::HashMap;

use super::BLOCK_LABEL;

/// Renders the largest prefix of `contracts` whose rendered block fits both the
/// configured `max_bytes` cap on the body and the conversation byte budget (what
/// is left of `byte_budget` after the system prompt and the user's prompt).
///
...
/// count the contracts (and so the claims) the byte bound dropped, for the
...
pub(super) fn bound_body(
    ...
    // body cap and the conversation byte budget. Both constraints are monotonic
    // in k (a longer body only grows), so the largest fitting k is well-defined.
    for k in (1..=contracts.len()).rev() {
        let body = super::render::render_body(&contracts[..k], name_of);
        if body.len() <= max_body_bytes
            && fits_message_budget(system_prompt, &body, prompt, byte_budget)
        {
...
/// prompt, still fits the conversation byte budget. Uses [`turn_bytes`] — the
/// exact post-escape, post-wrapper size `build_messages` enforces — so the
/// pre-build check and the build share one accounting. The block is measured
/// with `truncated: true` (the worst-case wrapper that includes the truncation
/// marker), so a block that fits here fits whether or not it is ultimately
/// marked truncated.
fn fits_message_budget(
    ...
    turn_bytes(system_prompt, std::slice::from_ref(&block), prompt) <= byte_budget
}
