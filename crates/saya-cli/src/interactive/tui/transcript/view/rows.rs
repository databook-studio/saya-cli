...
/// Wraps one logical line to `width` display cells into body [`Row`]s of
/// `kind` — a thin adapter over the shared cell-aware, grapheme-safe
/// [`wrap::wrap_cells`], which owns the wrapping semantics. The transcript's
/// callers pre-check empty input and emit bare blank rows themselves, so the
/// helper's empty-input row never doubles up here.
