...
// ---------------------------------------------------------------------------
// the runner-grant notice — a `runner:<program>` seed with nothing that can
// honour it is otherwise a silent no-op (the grant lands, but no
// `run_program` tool is ever offered, and the session never says why)
// ---------------------------------------------------------------------------

/// A launch's `runner:python3` seed with no `[jobs.runner]` composed at all
/// says so at startup, on the same notice seam `PROBE_REFUSED_NOTICE` rides
/// — naming the grant, the config keys that would compose it, and the
/// fallback (host commands still run, they just ask for approval).
#[test]
fn a_runner_grant_without_a_runner_says_so_at_launch() {
    let project = worktree("runner-notice-no-runner");
    let state = temp_dir("runner-notice-no-runner-state");
    let runtime = session_runtime(None);
    let launch = crate::interactive::session_host::HostLaunch::for_tests_seeded(
        vec!["runner:python3".to_owned()],
        &runtime,
    );
    let universe = SessionUniverse::compose_with_launch(
        &runtime,
    ...
    let notice = universe
        .notice
        .as_deref()
        .expect("a runner grant with no runner says so at launch");
    assert!(
...
/// A launch's `runner:python3` seed over a composed runner whose door
/// carries `python3` is silent — the grant is fully usable, nothing to say.
/// On a host whose probe refuses, the existing probe-refused notice stands
/// instead (no double message).
#[test]
fn a_runner_grant_with_a_composed_runner_is_silent() {
    let project = worktree("runner-notice-composed");
    let state = temp_dir("runner-notice-composed-state");
    let tools = temp_dir("runner-notice-composed-tools");
    let runtime = session_runtime(Some((vec!["python3".to_string()], Some(tools))));
    let launch = crate::interactive::session_host::HostLaunch::for_tests_seeded(
        vec!["runner:python3".to_owned()],
        &runtime,
    );
    ...
    .expect("composition succeeds on a plain worktree");
    if universe.runner.is_some() {
        assert!(
            universe.notice.is_none(),
            "a composed runner whose door carries the program is silent: {:?}",
            universe.notice
        );
    } else {
        use super::super::session_runner::PROBE_REFUSED_NOTICE;
        assert_eq!(
            universe.notice.as_deref(),
            Some(PROBE_REFUSED_NOTICE),
            "the probe's own notice stands, not a runner-gap one: {:?}",
            universe.notice
        );
    }
    let _ = (fs::remove_dir_all(&project), fs::remove_dir_all(&state));
...
/// A launch's `runner:python3` seed over a composed runner whose door
/// carries only `node` says so at startup, naming the program and the
/// config key it is missing from.
#[test]
fn a_runner_grant_for_a_program_outside_the_composed_allow_says_so_at_launch() {
    let project = worktree("runner-notice-not-in-allow");
    let state = temp_dir("runner-notice-not-in-allow-state");
    let tools = temp_dir("runner-notice-not-in-allow-tools");
    let runtime = session_runtime(Some((vec!["node".to_string()], Some(tools))));
    let launch = crate::interactive::session_host::HostLaunch::for_tests_seeded(
        vec!["runner:python3".to_owned()],
        &runtime,
    );
    ...
    if universe.runner.is_some() {
        let notice = universe
            .notice
            .as_deref()
            .expect("a program outside [jobs.runner] allow says so at launch");
        assert!(
...
/// Other scopes at launch never trigger the runner-grant notice: neither
/// `workspace-write` nor `command:git` is a `runner:` seed, so composing
/// with them states no runner fact at all.
#[test]
fn other_scopes_never_trigger_the_runner_notice() {
    let project = worktree("other-scopes-no-runner-notice");
    let state = temp_dir("other-scopes-no-runner-notice-state");
    let runtime = session_runtime(None);
    let launch = crate::interactive::session_host::HostLaunch::for_tests_seeded(
        vec!["workspace-write".to_owned(), "command:git".to_owned()],
        &runtime,
    );
    let universe = SessionUniverse::compose_with_launch(
        &runtime,
    ...
    assert!(
        universe.notice.is_none(),
        "no runner: seed, no runner notice: {:?}",
        universe.notice
    );
    let _ = (fs::remove_dir_all(&project), fs::remove_dir_all(&state));
}
